...
export async function saveOptionSchema(state: Options, documentId: string) {
	...
	try {
		...
		await connection();
		let document = await Setting.findOne({ _id: documentId }).lean().exec();
		if (!document) {
			return {
				// data: null,
				// success: null,
		...
		document = await Setting.findOne({ _id: documentId }).lean().exec();
		...
		return {
			data: JSON.parse(JSON.stringify(document?.optionsSchema)),
			success: true,
			error: false,
