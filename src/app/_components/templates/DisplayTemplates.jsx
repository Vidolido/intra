		...
		<div className='flex flex-wrap gap-6'>
			...
			{Object.entries(groupedItems).map(([collectionName, collection], i) => {
				...
				return (
					<section
						key={i}
						className={`flex justify-start gap-6 min-h-[200px] min-w-[200px] ${color.border}`}>
						<div
							className={`flex justify-center items-center min-w-14 ${color.bg} border border-white`}>
						...
						<div className='flex flex-col'>
							{collection.map((item, index) => {
								return (
									<Link
										key={index}
										href={`/dashboard/templates/${item._id}`}
