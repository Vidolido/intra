...
import { addTemplateRows } from '@/data-acceess/templates/addTemplateRows';
...
const InputForm = ({
  ...
  let handleAdd = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // console.log(mutCollections, ' mutCollections,');
    // let payload ={
    //   property
    // }

    // Тука чекам грешка
    await addTemplateRows({
      property,
      mutCollections,
      additional,
                ...
                {collection.map((item) => {
                  ...
                  return (
                    <label
                      ...
                      />
                      <span>
                        {typeOfValue(item, defaultLanguage, 'min', 'max')}
                      </span>
                    </label>
                  );
                })}
