...
import { editSetting } from '@/data-acceess/settings/editSetting';
...
const EditSettingsForm = ({
	...
	let parameterName = document.optionsSchema?.parameter;
	let collections = document.optionsSchema?.collections.map((coll) => ({
		...coll,
	...
	let isCollectionEmpty =
		state &&
		state?.collections &&
		state.collections[selectedCollection].length === 0;
	...
	const handleSubmit = async () => {
		...
		if (isEmpty) {
			...
			let collId = collections && collections[0]._id?.toString();
			if (response.success) {
				setInputType('simple');
				setState({
					parameter: {},
	...
	return (
        ...
        <ErrorMsg msg={actionStatus?.error?.collectionInput} />
			...
			{!isCollectionEmpty && (
				<div className='border border-slate-300 rounded p-1 mb-1'>
					<h5>Items</h5>
					<DisplayCollections
						languages={languages}
						state={state}
